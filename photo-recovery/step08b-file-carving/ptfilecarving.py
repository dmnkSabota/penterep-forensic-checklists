...
class PtFileCarving:
    ...
    Pipeline: load filesystem analysis JSON → check tools → PhotoRec carving →
    ...
    def load_fs_analysis(self) -> bool:
        """Load filesystem analysis JSON and extract image path."""
        ...
        if recommended == "filesystem_scan" and not self.force:
            return self._fail("fsAnalysisLoad",
                              "Filesystem analysis recommended filesystem_scan – "
                              "use Filesystem Recovery or --force to override.")
        if recommended == "hybrid":
            ptprint("Hybrid strategy – file carving will complement Filesystem Recovery.",
        ...
        ptprint(f"[OK] Loaded: method={recommended} | image={self.image_path.name}",
    ...
    def run_photorec(self) -> bool:
        ...
        ptprint("  [WARNING] Čas skenu závisí od veľkosti média – flash médiá 5–30 min, "
                "HDD (500 GB+) 2–8 hodín. Neprerušujte proces.",
                "WARNING", condition=not self.args.json)
        ...
        if not self.dry_run:
            ...
            cmd_file.write_text("\n".join(lines) + "\n", encoding="utf-8")
        ptprint(f"  Config: {len(IMAGE_FORMATS)} formats enabled (paranoid + expert mode).",
                "INFO", condition=not self.args.json)
        ...
        ptprint(f"{'[OK] PhotoRec completed' if ok else '[ERROR] PhotoRec failed'} "
    ...
    def run(self) -> None:
        ...
        self.ptjsonlib.add_node(self.ptjsonlib.create_node_object(
            "chainOfCustodyEntry",
            properties={
                "action": f"File carving dokončený – obnovených {s['unique']} súborov",
    ...
    def _write_text_report(self, props: Dict) -> Path:
        ...
        lines += ["", sep,
                  f"RECOVERED FILES (first 100 of {len(self._unique_files)}):", sep, ""]
...
def get_help() -> List[Dict]:
    return [
        ...
        {"usage_example": [
            ...
            "ptfilecarving PHOTORECOVERY-2025-01-26-001 --json",
        ...
        {"notes": [
            "Pipeline: analysis JSON → tools → PhotoRec → validate+dedup → EXIF+organise → report",
            "Output:   organized/ | corrupted/ | quarantine/ | duplicates/ | metadata/",
            "Filenames NOT preserved – renamed to {case_id}_{type}_{seq:06d}.{ext}",
            "Flash media (8–64 GB): 5–30 min | HDD (500 GB+): 2–8 h | Success rate: 50–65 %",
            "READ-ONLY: never modifies the forensic image",
            "Compliant with NIST SP 800-86 §3.1.2.3 and ISO/IEC 27037:2012",
        ]},
    ]
