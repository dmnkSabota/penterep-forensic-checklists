...
class PtRecoveryConsolidation:
    ...
    Pipeline: detect sources (FS-based recovery / File carving) → inventory
              → SHA-256 dedup → copy + organise → master_catalog.json + report.
    ...
    def detect_sources(self) -> List[str]:
        """Discover which recovery outputs exist."""
        ...
        if self.fs_recovery_dir.exists() and (
            ...
            ptprint(f"  [OK] FS-based recovery: {self.fs_recovery_dir.name}",
                    "OK", condition=not self.args.json)

        if self.carving_dir.exists() and (self.carving_dir / "organized").exists():
            sources.append("carved")
            ptprint(f"  [OK] File carving: {self.carving_dir.name}",
                    "OK", condition=not self.args.json)

        if not sources:
            return [] if self._fail(
                "sourceDetection",
                "No recovery sources found – run Filesystem Recovery and/or File Carving first."
            ) else []
    ...
    def run(self) -> None:
        ...
        self.ptjsonlib.add_node(self.ptjsonlib.create_node_object(
            "chainOfCustodyEntry",
            properties={
                "action": f"Konsolidácia dokončená – {s['unique']} unikátnych súborov, {s['dupes']} duplikátov odstránených",
        ...
        ptprint(f"Discovered: {s['discovered']} (fs={s['fs']}, carved={s['carved']}) | "
                ...
                "INFO", condition=not self.args.json)
        ptprint("Next: Photo Integrity Validation", "INFO", condition=not self.args.json)
        ptprint("=" * 70, "TITLE", condition=not self.args.json)
...
def get_help() -> List[Dict]:
    return [
        ...
        {"usage_example": [
            ...
            "ptrecoveryconsolidation PHOTORECOVERY-2025-01-26-001 --json",
